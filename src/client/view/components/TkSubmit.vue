...
export default {
  ...
  methods: {
    ...
    async uploadVoice (audioBlob) {
      try {
        ...
        // 获取文章路径和用户昵称
        const articlePath = getUrl(this.$twikoo.path) || 'unknown-path'
        const userNick = this.nick || 'anonymous'
        
        // 处理路径，移除特殊字符，确保路径安全
        const safeArticlePath = articlePath.replace(/[^\w\u4e00-\u9fa5\-_.]/g, '_').replace(/^\/+|\/+$/g, '')
        const safeUserNick = userNick.replace(/[^\w\u4e00-\u9fa5\-_.]/g, '_')
        
        // 构建新的文件名格式：文章路径/用户昵称/时间字符串.wav
        // 注意：不再添加pathPrefix前缀，因为服务器端会自动添加VOICE_CDN_PATH
        const fileName = `${safeArticlePath}/${safeUserNick}/${timeString}.wav`
  ...
  mounted () {
    ...
    // 详细检查并记录所有相关配置状态
    console.log('TkSubmit.vue - MOUNTED - config status:')
    console.log('  SHOW_VOICE:', this.config.SHOW_VOICE)
    console.log('  SHOW_EMOTION:', this.config.SHOW_EMOTION)
