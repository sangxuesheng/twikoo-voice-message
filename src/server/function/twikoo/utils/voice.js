...
const fn = {
  ...
  async uploadVoiceToQcloud ({ voice, fileName, config, res }) {
    ...
    // 生成文件路径，确保路径以/开头但不以/结尾
    let filePath = config.VOICE_CDN_PATH || '/twikoo'
    if (!filePath.startsWith('/')) {
      filePath = '/' + filePath
    }
    
    // 构建完整文件路径，fileName已经包含了完整的路径结构（twikoo/文章路径/用户昵称/时间字符串.wav）
    // 所以我们只需要移除fileName中的路径前缀部分，避免重复
    const pathPrefix = config.VOICE_CDN_PATH || 'twikoo'
    let fileRelativePath = fileName
    if (fileName.startsWith(pathPrefix)) {
      fileRelativePath = fileName.substring(pathPrefix.length)
    }
    // 确保路径以/开头
    ...
    const fullFilePath = filePath + fileRelativePath
