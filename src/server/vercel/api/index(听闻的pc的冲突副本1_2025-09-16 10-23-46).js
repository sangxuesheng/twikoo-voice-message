...
async function commentDeleteForAdmin (event) {
  ...
  if (isAdminUser) {
    ...
    // 查找评论
    const comment = await db.collection('comment').findOne({ _id: event.id })
    
    // 删除语音文件（如果存在）
    try {
      if (comment && comment.comment) {
        const voice = require('twikoo-func/utils/voice')
        await voice.deleteVoice(comment.comment, config)
      }
    } catch (e) {
      logger.error('删除语音文件失败:', e)
      // 语音文件删除失败不影响评论删除流程
    }
